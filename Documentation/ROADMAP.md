...
### ~~M5 — `ConversationStore` actor + turn verbs~~ ✅ **done 2026-07-28**
The concurrency boundary and the public write API (§6.5, §11). Still no FM — verbs take a driver, and M5 tests against a store-level scripted double, so the whole milestone verifies on a macOS 26 machine with zero Apple Intelligence eligibility. **Build order, decision log (D21–D29) and phase gates: [M5-PLAN.md](./M5-PLAN.md)** (drafted 2026-07-27 at the M4 boundary; **331 tests green**, 310 `LedgerKit` + 21 `Understudy`; **SPEC rev 8 ratified 2026-07-28** at this boundary — Appendix F, incl. the new §7.9 driver seam).

> **The driver seam is defined here, not at M6.** The store cannot take a concrete `GenerationDriver` (it does not exist until M6 and would drag the OS-coupled module into every store test) and must not take `any LanguageModel` (tenet 3). So M5 declares `GenerationDriving` — a `ModelDescriptor` plus one non-throwing call taking a request and a signal channel and returning the one terminal `Outcome` — and M6 supplies the single production conformance. **Non-throwing is the contract:** the store appends `generationStarted` before the call, so every later failure is an `Outcome` (§7.2), and cancellation *returns* `.cancelled`, which makes §11's documented deviation structural rather than documented.
>
> **Three findings worth carrying forward.** ① A **cancelled task cannot record its own cancellation** — a cancellation-aware backend makes writes inside a cancelled task throw, so the wind-down must run outside the cancelled scope or a stop erases its own evidence and the conversation reads `.interrupted`. ② §11's throw channel covers "**couldn't record**", not only "never started": a persistence failure mid-generation throws and deliberately writes no terminal. ③ **`LedgerError` gained a sixth case**, `unsupportedTarget` — `regenerate` of a root-level assistant would have to emit the nil-parent `generationStarted` that N10 reserves, so the store declines. All three are rev 8 items.
...
**Exit — all met:** ✅ the §11 API sketch **runs** end-to-end against a scripted driver, stop button and delete included (not merely compiles); ✅ single-flight and start atomicity have chaos tests — cancellation at every enumerated parked point × both stop mechanisms, plus the §7.2 straddle in all three of its positions; ✅ target eligibility; ✅ the two-channel contract with a log-untouched proof per throw condition; ✅ the new standing property — **every store-written log reduces with empty `diagnostics`**.
