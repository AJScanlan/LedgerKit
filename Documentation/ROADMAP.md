...
**M2.1 (`fold`) coverage against exit criteria.** I2/I6/I7 hold over every *prefix* of a rich and a hostile fixture, asserted as executable predicates rather than one-off expectations — the seed of M3's exhaustive small-scope enumeration, where only the input generator has to widen. I3/I4/I5 have direct unit tests (I5 at the fold layer means "stays `.open`" — synthesis is `classify`'s). I1 is pinned by a golden with literal orderings, deliberately not by repeating a fold in one process: Swift's hasher seed is fixed per process, so only a literal captured in one process and re-checked in later ones catches dictionary-order leakage. P3 is exercised at *every* split point of both fixtures. **Still M3's:** interior-gap fuzz variants, exact-residue hostile assertions row-for-row, and the version-frozen corpus.
