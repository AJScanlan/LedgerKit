...
- ~~`reduce ≡ classify ∘ fold` convenience.~~ **Done**, and the public entry point.
...
**Two §8 items left open** (neither blocks M3):
- **A `providerFailure` status outside 4xx/5xx is not in §8's table.** Implemented as `providerUnclassified` (`terminal`, the safe default) on the grounds that such a failure is malformed rather than meaningful. Worth a spec row.
- **"Logged loudly" has no home in a pure reducer.** §8 annotates four classification rows that way, but `classify` cannot log. Three of them are satisfiable at *normalization* time in the driver (M6), where `unrecognized` and nil-status/nil-code `providerFailure` are actually created. The exception is "unmatched provider `code`," which only classification can detect — needing either provenance in the return type or acceptance. Deferred deliberately, but note that adding provenance later is source-breaking for the one function everything calls.
...
- **Two-stage decode in the loader**, emitting `LoadedEvent` (moved here from M2). Decode the envelope first, then the payload: M1's `Record.init(from:)` decodes the payload with a plain `try`, so an unknown discriminator discards the whole record, envelope included, and every §6.6 row-2 diagnostic degrades to sequence-only. Also: an unreadable row must be **emitted, not dropped** — dropping it turns a row-1/2 condition into a *gap* diagnostic, which is a different and false claim (§6.6 input corollary).
