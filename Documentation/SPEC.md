...
**The formal-methods opportunity is real and it is not here (rev 10).** Rev 4 suggested modelling I1–I7 in TLA+/PlusCal and singled out "model-checking I5 against random truncation" as exactly what TLC is for. Both halves were testable claims and both came out the other way. The reducer is pure, total and single-threaded, and is already swept **exhaustively in the real code** — every fixture, every truncation, every interior gap, and bounded-exhaustive generated logs besides — so a model of it would restate the reducer, add an abstraction gap, and buy only the shape diversity that Swift generation already buys without one. Where a model checker did pay is the layer this section does not mention: the **store's interleavings** (§6.5, §9), where TLC reproduced a real delete-versus-start bug in ten states and then *falsified the remedy that had been proposed for it*. The encoding that makes this cheap is one rule — **a PlusCal label is an `await`** — so Swift actor reentrancy transcribes into PlusCal rather than being interpreted into it. One discipline is load-bearing: a model is trustworthy only after it reproduces a bug already known to be real, because a model too weak to express the bug passes everything and looks exactly like success.
...
Writing every token to disk is wasteful; losing 30 s of stream on crash is bad UX. The **store** coalesces *disk* flushes on a policy (default: every ~250 ms or N chars, and always before `generationEnded`). **Rev 8 corrects the attribution, not the behaviour:** rev 2 said "driver", written before the driver seam existed (§7.9). The store owns every append, so the *cadence* of appends is necessarily its business too; what the driver owns is what this section actually cared about — producing deltas rather than snapshots, which is the diffing on the far side of the seam. The *observable projection* applies deltas as they arrive, in memory, ahead of disk — so streaming renders at display cadence while the log fills at durability cadence. **"Display cadence" is the observation system's guarantee, not a timer the projection runs (rev 10).** `@Observable` already coalesces invalidations to about a frame, so the projection's own cadence knob buys no additional smoothness; what it buys is less *work* — one overlay application per coalescing window rather than one per delta. Its default is therefore **immediate**, and raising it is a CPU-for-latency trade an app makes after measuring, not a prerequisite for smooth rendering. Rev 2 wrote the projection as `fold(persisted log) ⊕ unflushed tail` and left `⊕` undefined; it does more work than "append." Precisely:
...
**A failed invocation is recorded too, and that obligation is the driver's (rev 10).** The framework reports a successful invocation as *data* — a `toolOutput` entry — and a failed one as a *thrown error*, so the two arrive through different doors and a driver that watches only the transcript sees only successes. The driver must therefore emit `toolInvocationRecorded` with `status: .failed` from its own catch path, before the terminal: name from the `ToolCallError` wrapper, duration where the call was observed, payloads per policy. This is what makes §8's "two facts, two events" literally true rather than aspirational — §8 unwraps the wrapper and classifies what is underneath, which discards the one thing only the wrapper knew. Until M7 the sentence was false and `ToolRecord.Status.failed` was wire surface no code could reach.

⚠️ **A failed invocation's duration is frequently `nil`, and that is measurement rather than omission (rev 10).** Timing is measured from the driver's own observation of the call, so it exists only if a snapshot carrying the `toolCalls` entry arrived before the throw. Measured at M7: when the tool call is the model's **first** action, no snapshot precedes it and nothing about the call is ever surfaced — the record then carries the name and the status and nothing else. Attribution is also declined when it would be a guess: two concurrent calls to the same tool cannot be told apart by a wrapper that names only the tool, so the duration is `nil` rather than assigned to whichever was found first. `nil` means *not reported*, which is what it means everywhere else in this spec.
...
⚠️ **Watch-note for v0.2 (rev 10).** Apple's own deprecation chain names `GeneratedContent.ParsingError` as the replacement for the 26 family's `decodingFailure`, and this taxonomy deliberately lands them on different codes — `emptyResponse` and `decodingFailure`. That is right for v0.1, which requests plain `String` only, so a `ParsingError` can only mean zero tokens. It stops being right the moment guided generation is requested (N8, v0.2): `ParsingError` will then carry two distinct conditions, and a code naming one of them will mislead triage. Split it at that point, not before.
...
// The observable read side — §11's isolation sketch below. A @MainActor
...
// SwiftUI — message states drive UI directly, and `.streaming` can only ever
// appear here (§6.2): no fold of any log yields it.
...
// An app overriding §8's affordance table does it on the PROJECTION, not the
// store: `Recoverability` is never persisted, so the override applies at
// classification time and retroactively upgrades historical failures.
let lenient = try await ConversationProjection(of: convo.id, in: store, mapping: myMapping)

// Conversation list — the index projection, not N reductions. Its own type,
// not a property of the above: rev 10 records that a single facade owning both
// was considered and declined (it would hold projections for conversations
// nobody is displaying, or grow a cache policy nobody asked for).
let list = try await ConversationListProjection(in: store)
ForEach(list.conversations) { summary in ... }
```

Both projection initializers are `async throws` because attaching *reads the log* and the conversation may not exist. That is deliberate rather than incidental (rev 10): a synchronous initializer would have to publish a `Conversation` that is not yet a reduction of anything, and the only honest spelling of that is an `Optional` whose `nil` conflates "still loading" with "deleted" with "the disk failed" — three conditions with three different responses. Paying once at construction is what lets `conversation` be non-optional at every render site, with a separate terminal flag for the deletion case (§9).
...
**Isolation sketch (tenet 6):** `ConversationStore` is an actor and owns all writes. The observable projection is a `@MainActor @Observable` class fed by the store; deltas hop to the main actor as they arrive, independent of the *disk* flush cadence (§7.4). "About a frame" is what `@Observable` coalescing already provides, and the projection's own cadence knob is a work-reduction valve on top of it (rev 10) — not the mechanism that makes rendering smooth. The reducer is pure, `nonisolated` functions over `Sendable` values — no isolation, no clocks, no I/O. The liveness overlay (§7.4) is applied on the projection side — the reducer never learns what "live" means.
...
- "The model checker found the bug my audit missed — and then killed my fix" — TLC against the store's interleavings (§6.5/§9), including the calibration rule and the label-is-an-`await` encoding. A better post than the invariants one rev 4 imagined, because it has a result.
...
## Appendix H — Changes from rev 9

Rev 10 is opened by **M7** (the observable read side) and ratified at its boundary,
**2026-08-16**. Thirteen items in five batches. **No invariant weakens, no event kind
is added or removed, and nothing touches the wire** — no payload shape and no
discriminator changed, confirmed item by item.

**Its character is worth naming, because it differs from every revision before it.**
Rev 6 and rev 7 were *reading the SDK*. Rev 9 was *measurements that contradicted what
a reader would reasonably have believed*. Rev 10 is mostly **this document being wrong
about its own tests and tools**: four items (the §6.3 aside, §6.5's enforcement point,
§7.4's cadence, §10.6's P2) exist because something stated here was checked against
running code and did not hold. That is a less flattering source than an SDK surprise
and a more useful one — a spec can be wrong about a vendor and stay internally sound,
but a spec wrong about its own coverage is telling its readers the wrong thing about
what is guaranteed.

Two items were found by **mutation testing** rather than by reading, and both were
claims of coverage: `Corpus.all` contained no failed generation at all, so nothing
anywhere had ever swept the most complex `MessageState` case; and P2's clause 1 is
tautological against a live store, so a wrong live set satisfied it.

- **§7.6 — a failed invocation must be recorded, and that is the driver's obligation.**
  §8 has said since rev 9 that unwrapping `ToolCallError` loses nothing because "a tool
  failed" has its own channel. That was **false**: the driver emitted records only from
  `toolOutput` entries, which exist only on success, so `ToolRecord.Status.failed` was
  wire surface no code could reach. The framework reports success as *data* and failure
  as a *thrown error*, so the two arrive through different doors and a driver watching
  only the transcript sees only successes. §7.6 now states the obligation, which is what
  makes §8's sentence true.
- **§7.6 — a failed invocation's duration is frequently `nil`, by measurement.** Timing
  comes from the driver's own observation of the call, so it exists only if a snapshot
  carrying the `toolCalls` entry arrived before the throw — and when the tool call is
  the model's *first* action, none does. Attribution is also declined rather than
  guessed when two concurrent calls share a tool name. `nil` means *not reported*,
  which is what it means everywhere else here.
- **§7.4 / §11 — "display cadence" is the observation system's guarantee, not a timer
  the projection runs.** Rev 2's wording implied the projection must coalesce to a
  frame to render smoothly. `@Observable` already does that, so the projection's own
  cadence knob buys **less work**, not more smoothness — and its default is therefore
  *immediate*. Measured at M7 by counting overlay applications at both settings, which
  is the difference between a knob that is justified and one that is assumed.
- **§7.4 — attaching mid-generation shows the live partial.** A projection created
  while a generation streams renders `.streaming` with the whole partial, not
  `.interrupted` and not merely the flushed prefix. Stated because it is the ordinary
  case (list → detail navigation) and because getting it wrong is **invisible to P2**:
  the predicate checks a projection against the live set it was handed, and an empty
  live set is self-consistent. M7 shipped that bug and only a running pipeline found
  it. The same sentence covers a generation that has started and produced no text —
  `.streaming(partial: "")`, which is the case §6.2's refusal of a distinct `.pending`
  was written for and which nothing had previously reached.
- **§9 — deletion is surfaced to the read side rather than thrown at it.** A projection
  over a deleted conversation reports the deletion and freezes its last view. §9 already
  called deletion irreversible and out-of-band; what it did not say is what the *read*
  side does about it, and the answer must not be "infer a lifecycle fact from a failed
  read" (tenet 1).
- **§11 — the sketch gains the read side, and records one divergence.** Both projections,
  the mapping override riding the projection, and the exhaustive switch over
  `projection.conversation`. ⚠️ Rev 9 sketched the list as `projection.conversationList`,
  implying **one facade** owning both; that facade was considered and declined at M7 (it
  would hold projections for conversations nobody is displaying, or grow a cache policy
  nobody asked for), so the spelling is two types. Also recorded: both initializers are
  `async throws` **deliberately** — attaching reads the log and the conversation may not
  exist, and a synchronous initializer would have to publish an `Optional` whose `nil`
  conflates loading, deleted and disk-failure.
- **§6.5 / §9 — the healthy-log property gains an enforcement point.** Rev 8 established
  that store-written logs never quarantine and left it a claim about store *discipline*.
  M7 found that an **interleaving** — not a coding slip — could make the store write a
  log that quarantines under §6.6 row 5 forever. §9's write boundary now refuses a batch
  that would be a conversation's first rows without being its genesis, inside the append
  transaction, which is the only place where "does this conversation have rows" and "am I
  adding rows" are answered *of the same instant*. The generalizable finding: **no
  in-memory guard was the right unit**, because the property is about logs and an
  in-memory flag describes one process's memory. A model checker established that, and
  then falsified the in-memory remedy that had been proposed.
- **§6.3 / Appendix A — the TLA+ aside pointed at the wrong layer.** Rev 4 suggested
  modelling I1–I7 and singled out I5 against random truncation. Both halves came out the
  other way: the reducer is pure, total, single-threaded and already swept exhaustively
  *in the real code*, so a model would restate it and add an abstraction gap. Where TLC
  paid is the layer §6.3 did not mention — the store's interleavings — where it
  reproduced a real bug in ten states and killed the proposed fix. The encoding that
  makes it cheap is one rule: **a PlusCal label is an `await`.** And one discipline is
  load-bearing: a model is trustworthy only after it reproduces a bug already known to be
  real, because a model too weak to express the bug passes everything and looks exactly
  like success.
- **§10.6 — P2's clause 1 is scoped honestly.** Rev 4 phrased it against the *log*
  ("equal to the concatenated deltas"); the harness compares against the **live set**,
  which is the only thing available when the live set is synthetic and is a *tautology*
  against a live store, because the overlay constructs what it shows from the live set.
  Found by a mutation that survived the predicate. Clause 1 is now the overlay's fidelity
  to its input; the stronger claim is the **store's** obligation and needs a source
  neither layer computed. Both are tested — they are different properties, and one clause
  claiming both is how the mutation went unnoticed.
- **§7.8 — the cardinality parenthetical described headroom as if shipped.** "(an
  internal cache keyed by `ConversationID`)" read as implementation; v0.1 rebuilds per
  generation and no cache exists — which is *why* there is nothing for
  `deleteConversation` to leave stale. Reworded to allowance-plus-reality.
- **§12 — cut line 4 versus its invocation.** The line priced "on-device +
  Claude-package only"; the invocation shipped no Claude package and *wider* Apple-side
  breadth (§8's second table). Annotated invoked-with-outcome, since a cut line that
  does not record how it actually went is a worse guide than none.
- **§13 — DoD-2 restated against a second real provider.** The demonstrable one is
  `PrivateCloudComputeLanguageModel`, because cut line 4 established the Claude package
  is not in this beta ring. **The product claim was always the one-line swap, not the
  vendor.**
- **§8 — a watch-note on `emptyResponse` versus `decodingFailure`.** Apple's deprecation
  chain names `GeneratedContent.ParsingError` as `decodingFailure`'s replacement, and
  this taxonomy lands the two on different codes. Correct while v0.1 requests plain
  `String` only — a 27 `ParsingError` can then only mean zero tokens — and wrong the
  moment guided generation is requested. One sentence now prevents a confused triage in
  v0.2.

**Not amended, deliberately.** The store→projection feed's notification shapes, its
fan-out, and its ordering guarantee are **internal** and stay unsaid: the spec says the
projection is "fed by the store" and that is the right level of commitment. §6.2's
`.streaming(partial: "")` needed no new text — the sentence was already right and is now
merely *exercised*. ADR-003's rule 4 was amended separately (an ADR edit, not a spec
edit) to record that value observation was examined at M7 and **declined**, with the
