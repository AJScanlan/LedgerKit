...
- **Status:** M0–**M5** done and **M6 Phase 0 landed (2026-07-29)**, **335 tests green** (314 `LedgerKit` + 21 `Understudy`, warning-free; the 314 also green on the iOS 27 simulator). **SPEC rev 8 ratified 2026-07-28** at the M5 boundary (Appendix F: two M4-audit items + nine from M5, including the new **§7.9 driver seam**); further amendments open **rev 9** (ratifies at the M6 boundary). **ADR-003 Accepted; ADR-001 D-1/D-2/D-3 all closed.** **In progress: M6 — the `Session/` seam**, the one OS-coupled module; plan and decision log **`Documentation/M6-PLAN.md` (D30–D37)**, next up **Phase 1** (the pure differ + normalization cores). Cold-open criterion met twice: 10,004 events → 3 rows replayed (M4), and a store-triggered refresh now replays **zero** (M5). `Core/`, `Reduce/` and `Store/` are complete; `Session/` and `Projection/` are still empty. Public reduction entry point is `Conversation(reducing:loadedFrom:mapping:)` — there is no top-level `reduce`. Plans and decision logs: M4 (D13–D20) in `Documentation/M4-PLAN.md`, M5 (D21–D29) in `Documentation/M5-PLAN.md`, **M6 (D30–D37) in `Documentation/M6-PLAN.md`**.
