...
- **`Documentation/SPEC.md`** — the **contract** (rev 7, **ratified** 2026-07-26 at the M4 boundary). Semantics defined here are binding; type names in it are illustrative ("bikesheddable; semantics not").
...
- **Test rhythm**
  ...
  - Reducer/spec work goes: audit → propose SPEC amendment → get approval → implement. **Rev 7 is ratified, so amendments now open rev 8** (new appendix, new `Changes from rev 7:` header) rather than editing rev 7 in place. The drafting pattern that worked at rev 7: write the proposed edits to a scratch draft, get sign-off item by item, *then* touch `SPEC.md`.
