...
- Toolchain: **Xcode 27.0 Beta 4** (build 27A5228h), **macOS 27.0 SDK**, Swift 6 language mode, strict concurrency. Packages deploy to 26 (`.macOS(.v26)`), so iOS/macOS 27 APIs need `@available` guards — they *compile* (the SDK is 27) but the dev machine runs macOS 26, so 27-only code cannot execute here. Gate such tests with `.enabled(if:)`. ⚠️ **Never bump a package floor to 27** — a 27-floor test binary cannot launch on this macOS 26 host, which strands the *entire* suite, not just the gated tests. `Session/` is availability-gated instead, exactly like `Understudy`'s `ScriptedLanguageModel` (M6-PLAN D31).
...
- **Status:** M0–**M5** done, **331 tests green** (310 `LedgerKit` + 21 `Understudy`), **SPEC rev 8 ratified 2026-07-28** at the M5 boundary (Appendix F: two M4-audit items + nine from M5, including the new **§7.9 driver seam**); further amendments open **rev 9** (ratifies at the M6 boundary). **ADR-003 Accepted; ADR-001 D-1/D-2/D-3 all closed.** **Next: M6 — the `Session/` seam**, the one OS-coupled module; **plan drafted: `Documentation/M6-PLAN.md` (D30–D37)**. ⚠️ **The M5 boundary audit (2026-07-28) found two store bugs, fixed at M6 Phase 0 — until that lands, know them:** `deleteConversation` doesn't wait for a *claimed-but-unconfirmed* start (`.reserved` slot), so it can race the start append; and the rehydration read between the start commit and the driver launch sits outside every `release` guard, so a throw there wedges single-flight and a task-cancel there escapes as a post-append `CancellationError` (§7.2 violation). Phase 0 also rewrites the stale pre-rev-8 throw-channel docs on `LedgerError`/the verbs and makes the flush/snapshot policies publicly constructible (D32). Cold-open criterion met twice: 10,004 events → 3 rows replayed (M4), and a store-triggered refresh now replays **zero** (M5). `Core/`, `Reduce/` and `Store/` are complete; `Session/` and `Projection/` are still empty. Public reduction entry point is `Conversation(reducing:loadedFrom:mapping:)` — there is no top-level `reduce`. Plans and decision logs: M4 (D13–D20) in `Documentation/M4-PLAN.md`, M5 (D21–D29) in `Documentation/M5-PLAN.md`, **M6 (D30–D37) in `Documentation/M6-PLAN.md`**.
