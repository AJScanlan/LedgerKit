...
- **Status:** M0–M3 done and **audited**, **M4 Phases 0–1 done**, **223 tests green** (202 `LedgerKit` + 21 `Understudy`), **SPEC rev 6 ratified** (2026-07-26; amendments now open rev 7). **ADR-003 Accepted; ADR-001 D-1/D-2 closed** (D-3, the registry manifest, is M4 Phase 4). `Core/` and `Reduce/` are complete. Public reduction entry point is `Conversation(reducing:loadedFrom:mapping:)` — there is no top-level `reduce`. `Store/Persistence.swift` is the *seam only* — no GRDB wiring; its `events` verb returns `[LoadedEvent]`, which is what makes M4's two-stage decode implementable above the seam. `Session/` and `Projection/` are still empty. **Next: M4** (SQLite store, snapshots, index) — plan in `Documentation/M4-PLAN.md`; decision numbering continues M3's at D13.
