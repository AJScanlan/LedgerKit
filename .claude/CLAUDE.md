...
- **Status:** M0–M3 done and **audited**, **M4 Phases 0–4 done**, **266 tests green** (245 `LedgerKit` + 21 `Understudy`), **SPEC rev 6 ratified** (2026-07-26; amendments now open rev 7). **ADR-003 Accepted; ADR-001 D-1/D-2/D-3 all closed** — that ADR has no open questions left. **Next: M4 Phase 5** (the wrap-up: draft SPEC rev 7 from M4-PLAN §6 — *not* from memory — ratify it, record the ADRs closed, update ROADMAP + this file). Cold-open criterion met: 10,004 events → 3 rows replayed. `Core/` and `Reduce/` are complete. Public reduction entry point is `Conversation(reducing:loadedFrom:mapping:)` — there is no top-level `reduce`. `Store/Persistence.swift` is the *seam only* — no GRDB wiring; its `events` verb returns `[LoadedEvent]`, which is what makes M4's two-stage decode implementable above the seam. `Session/` and `Projection/` are still empty. **Next: M4** (SQLite store, snapshots, index) — plan in `Documentation/M4-PLAN.md`; decision numbering continues M3's at D13.
